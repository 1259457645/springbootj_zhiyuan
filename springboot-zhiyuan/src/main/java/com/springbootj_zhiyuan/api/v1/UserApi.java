...
public class UserApi {
    ...
    public Object login( @RequestBody User user){
        ...
        if(userForBase==null){
            ...
            if (!BCrypt.checkpw(user.getPassword(), userForBase.getPassword())){
                jsonObject.put("code","500");
    ...
    public Object updatePassword(@PathVariable("id")String id,@RequestBody User user){
        JSONObject jsonObject=new JSONObject();
        String encodePwd = BCrypt.hashpw(user.getPassword(), BCrypt.gensalt()); // 加密
        user.setPassword(encodePwd);
        int res=userService.updatePw(id,user.getPassword());
        if(res>0){
            jsonObject.put("code","200");
